...
namespace diskann {
  ...
  void Index<T, TagT>::load(const char *filename, const bool load_tags,
    ...
    std::string tmp("toggkdt_");
    std::string togg_filename(tmp.append(filename));
    std::ifstream togg_in(togg_filename.c_str(), std::ios::binary);
    if (!togg_in.is_open()) {
    Tn.resize(_nd);
    r_proportion = 1;
    float* max = new float[(size_t)_dim];
    float* min = new float[(size_t)_dim];
    togg_range = new float[(size_t)_dim]; 
    for (size_t i = 0; i < _dim; i++) {
      max[i] = min[i] = _data[i];
    }
    for (size_t i = 0; i < _dim; i++) {
      for (size_t j = i; j < _nd * _dim; j += _dim) {
        if(max[i] < _data[j]) max[i] = _data[j];
        if(min[i] > _data[j]) min[i] = _data[j];
      }
    }
    for (size_t i = 0; i < _dim; i++) {
      togg_range[i] = max[i] - min[i];
    }
    for (size_t i = 0; i < _nd; i++) {
      auto size = _final_graph[i].size();

      long long min_diff = 1e6, min_diff_dim = -1;
      for (size_t j = 0; j < _dim; j++) {
        int lnum = 0, rnum = 0;
        for (size_t k = 0; k < size; k++) {
          if (*(_data + _final_graph[i][k] + j) - *(_data + i + j) < -r_proportion * togg_range[j])
            lnum++;
          else if (*(_data + _final_graph[i][k] + j) - *(_data + i + j) >= r_proportion * togg_range[j])
            rnum++;
        }
      ...
      for (size_t k = 0; k < size; k++) {
        if (*(_data + _final_graph[i][k] + min_diff_dim) - *(_data + i + min_diff_dim) < -r_proportion * togg_range[min_diff_dim])
          Tn[i].left.push_back(_final_graph[i][k]);
        else if (*(_data + _final_graph[i][k] + min_diff_dim) - *(_data + i + min_diff_dim) >= r_proportion * togg_range[min_diff_dim])
          Tn[i].right.push_back(_final_graph[i][k]);
        else
          Tn[i].common.push_back(_final_graph[i][k]);
      }
    }

      size_t index_size = 0;
      std::ofstream out(togg_filename.c_str(), std::ios::binary);
      out.write((char *) &index_size, sizeof(_u64));
      out.write((char *) &_width, sizeof(unsigned));
      out.write((char *) &r_proportion, sizeof(unsigned));
      out.write((char *) &_ep, sizeof(unsigned));
      for (unsigned i = 0; i < _nd + _num_frozen_pts; i++) {
      ...
      size_t        expected_file_size;
      togg_in.read((char *) &expected_file_size, sizeof(_u64));
      togg_in.read((char *) &_width, sizeof(unsigned));
      togg_in.read((char*)&r_proportion, sizeof(float));
      togg_in.read((char *) &_ep, sizeof(unsigned));
      ...
      while (!togg_in.eof()) {
        ...
        Tnode tmp;
        togg_in.read((char*)&tmp.div_dim, sizeof(unsigned));
        togg_in.read((char*)&left_len, sizeof(unsigned));
        togg_in.read((char*)&common_len, sizeof(unsigned));
        togg_in.read((char*)&right_len, sizeof(unsigned));
        if (togg_in.eof()) break;
        cc += (left_len + right_len + common_len);
        ++nodes;
        tmp.left.resize(left_len);
        tmp.common.resize(common_len);
        tmp.right.resize(right_len);
        togg_in.read((char*)tmp.left.data(), left_len * sizeof(unsigned));
        togg_in.read((char*)tmp.common.data(), common_len * sizeof(unsigned));
        togg_in.read((char*)tmp.right.data(), right_len * sizeof(unsigned));
        Tn.push_back(tmp);
      }
      if (_final_graph.size() != _nd) {
        diskann::cout << "ERROR. mismatch in number of points. Graph has "
...
#pragma omp parallel for schedule(dynamic, 65536)
  ...
  void Index<T, TagT>::optimize_graph() {  // use after build or load
    ...
    _neighbor_len = (_width + 4) * sizeof(unsigned);
    ...
    for (unsigned i = 0; i < _nd; i++) {
      ...
      cur_node_offset += sizeof(unsigned);
      std::memcpy(cur_node_offset, Tn[i].left.data(), left_len * sizeof(unsigned));
      cur_node_offset += left_len * sizeof(unsigned);
      std::memcpy(cur_node_offset, Tn[i].common.data(), common_len * sizeof(unsigned));
      cur_node_offset += common_len * sizeof(unsigned);
      std::memcpy(cur_node_offset, Tn[i].right.data(), right_len * sizeof(unsigned));
      cur_node_offset += right_len * sizeof(unsigned);
    }
  }
#else
  template<typename T, typename TagT>
...
//    boost::dynamic_bitset<> flags{_nd, 0};
    ...
    while (k < (int) L) {
      ...
      if (retset[k].flag) {
        ...
        unsigned div_dim = *neighbors;
        unsigned left_len = *(neighbors + 1);
        unsigned common_len = *(neighbors + 2);
        unsigned right_len = *(neighbors + 3);
        neighbors += 4;
        unsigned MaxM;
        if (query[div_dim] - neighbors[div_dim] < -r_proportion * togg_range[div_dim]) {
          MaxM = left_len + common_len;
        }
        else if (query[div_dim] - neighbors[div_dim] >= r_proportion * togg_range[div_dim]) {
