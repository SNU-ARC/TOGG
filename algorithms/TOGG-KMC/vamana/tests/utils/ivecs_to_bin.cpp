...
void block_convert(std::ifstream& reader, std::ofstream& writer, _u32* read_buf,
                   _u32* write_buf, _u64 npts, _u64 ndims, _u32 groundtruth_id) {
  ...
  for (_u64 i = 0; i < npts; i++) {
    for (_u64 j = 0; j < ndims; j++) {
      *(write_buf + i * ndims + j) -= 6250000 * groundtruth_id;
    }
  }
...
int main(int argc, char** argv) {
  if (argc != 3 || argc != 4) {
    std::cout << argv[0] << " input_ivecs output_bin [optional]" << std::endl;
    exit(-1);
  }
  ...
  _u32 groundtruth_id = std::atoi(argv[3]);
  for (_u64 i = 0; i < nblks; i++) {
    _u64 cblk_size = std::min(npts - i * blk_size, blk_size);
    block_convert(reader, writer, read_buf, write_buf, cblk_size, ndims, groundtruth_id);
