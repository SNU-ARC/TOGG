...
vamana_deep100M_16T() {
  ...
  for id in ${sub_num[@]}; do
    awk 'NR==13{ print $0; exit }' deep100M_search_L${1}K${2}_${3}_T${4}_${id}.log >> deep100M_search_L${1}K${2}_${3}_T${4}.log
  done
  ...
  if [ "${1}" == "sift1M" ]; then
    ...
    for k in ${K[@]}; do
      for l_size in ${L_SIZE[@]}; do
        declare -i l=l_size
        for t in ${THREAD[@]}; do
          vamana_deep100M_1T ${l} ${k} baseline ${t}
        done
      done
    done
    ...
    for k in ${K[@]}; do
      for l_size in ${L_SIZE[@]}; do
        declare -i l=l_size
        for t in ${THREAD[@]}; do
          vamana_deep100M_16T ${l} ${k} baseline 16
        done
      done
    done
  elif [ "${1}" == "all" ]; then
