...
vamana_sift1M() {
  ...
  if [ ! -f "sift1M.index" ]; then
    ...
    ./build_memory_index float l2 ../../../../../routing_evaluation/dataset/sift1M/sift_base.fvecs.bin sift1M.index 70 75 1.2 ${MAX_THREADS} # [L] [R] [alpha] [num_threads]
  fi
...
vamana_gist1M() {
  ...
  if [ ! -f "gist1M.index" ]; then
    echo "Generating Vamana index"
    ./build_memory_index float l2 ../../../../../routing_evaluation/dataset/gist1M/gist_base.fvecs.bin gist1M.index 70 75 1.2 ${MAX_THREADS} # [L] [R] [alpha] [num_threads]
  fi
...
vamana_crawl() {
  ...
  if [ ! -f "crawl.index" ]; then
    echo "Generating Vamana index"
    ./build_memory_index float l2 ../../../../../routing_evaluation/dataset/crawl/crawl_base.fvecs.bin crawl.index 70 75 1.2 ${MAX_THREADS} # [L] [R] [alpha] [num_threads]
  fi
...
vamana_deep1M() {
  ...
  if [ ! -f "deep1M.index" ]; then
    echo "Generating Vamana index"
    ./build_memory_index float l2 ../../../../../routing_evaluation/dataset/deep1M/deep1m_base.fvecs.bin deep1M.index 70 75 1.2 ${MAX_THREADS} # [L] [R] [alpha] [num_threads]
  fi
...
vamana_msong() {
  ...
  if [ ! -f "msong.index" ]; then
    echo "Generating Vamana index"
    ./build_memory_index float l2 ../../../../../routing_evaluation/dataset/msong/msong_base.fvecs.bin msong.index 30 40 2 ${MAX_THREADS} # [L] [R] [alpha] [num_threads]
  fi
...
vamana_glove-100() {
  ...
  if [ ! -f "glove-100.index" ]; then
    echo "Generating Vamana index"
    ./build_memory_index float l2 ../../../../../routing_evaluation/dataset/glove-100/glove-100_base.fvecs.bin glove-100.index 70 75 1.2 ${MAX_THREADS} # [L] [R] [alpha] [num_threads]
  fi
...
vamana_deep100M() {
  ...
  if [ ! -f "deep100M.index" ]; then
    echo "Generating Vamana index"
    ./build_memory_index float l2 ../../../../../routing_evaluation/dataset/deep100M/deep100M_base.fvecs.bin deep100M.index 70 75 1.2 ${MAX_THREADS} # [L] [R] [alpha] [num_threads]
  fi
...
vamana_deep100M_16T() {
  ...
  for id in ${sub_num[@]}; do
    ...
    if [ ! -f "deep100M_${id}.index" ]; then
      echo "Generating Vamana index"
      ./build_memory_index float l2 ../../../../../routing_evaluation/dataset/deep100M/deep100M_base_${id}.fvecs.bin deep100M_${id}.index 70 75 1.2 ${MAX_THREADS} # [L] [R] [alpha] [num_threads]4
    fi
  done
  echo "Perform searching using Vamana index (deep100M_L${1}K${2}T${3})"
  ...
  sudo sh -c "sync && echo 3 > /proc/sys/vm/drop_caches"
  for id in ${sub_num[@]}; do
    ./search_memory_index float fast_l2 ../../../../../routing_evaluation/dataset/deep100M/deep100M_base_${id}.fvecs.bin deep100M_${id}.index 1 ../../../../../routing_evaluation/dataset/deep100M/deep100M_query.fvecs.bin \
      ../../../../../routing_evaluation/dataset/deep100M/deep100M_groundtruth_${id}.ivecs.bin ${2} deep100M_search_L${1}K${2}T${3}_${id}_${3} ${1} > \
