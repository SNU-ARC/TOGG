...
namespace diskann {
...
#ifdef TOGG_KMC
  template<typename T, typename TagT>
  void Index<T, TagT>::init_cluster_para(const size_t cl_num) {
    cluster_num = cl_num;
    m_means = new float*[cluster_num];
    for (size_t i = 0; i < cluster_num; i++) {
      m_means[i] = new float[_dim];
      std::memset(m_means[i], 0, sizeof(float) * _dim);
    }
  ...
  template<typename T, typename TagT>
  void Index<T, TagT>::init(unsigned* m_neighbors, int N) {
    int size = N;
    if (m_initMode == InitRandom) {
      int inteval = size / cluster_num;
      float* sample = new float[_dim];
      srand((unsigned)time(NULL));

      for (size_t i = 0; i < cluster_num; i++) {
        int select = inteval * i + (inteval - 1)  * rand() / RAND_MAX;
        for (size_t j = 0; j < _dim; j++) {
          sample[j] = *(_data + (*(m_neighbors + select)) * _dim + j);
        }
        std::memcpy(m_means[i], sample, sizeof(float) * _dim);
      }
    }
  ...
  template<typename T, typename TagT>
  float Index<T, TagT>::get_label(float* sample, int* label, std::vector<bool> &cflag) {
    DistanceFastL2<T> *dist_fast = (DistanceFastL2<T> *) _distance;
    float dist = -1;

    for (size_t i = 0; i < cluster_num; i++) {
      float temp = togg_dist->compare(sample, m_means[i], _dim);
  ...
  template<typename T, typename TagT>
  void Index<T, TagT>::Cluster(unsigned* m_neighbors, int N, size_t *Label) {
    int size = 0;
    size = N;
    ...
    float** next_means = new float*[cluster_num];
    for (size_t i = 0; i < cluster_num; i++) {
      next_means[i] = new float[_dim];
    }
    while (loop) {
      std::memset(counts, 0, sizeof(int) * cluster_num);
      for (size_t i = 0; i < cluster_num; i++) {
        std::memset(next_means[i], 0, sizeof(float) * _dim);
      }
      lastCost = currCost;
      currCost = 0;
      ...
      for (int i = 0; i < size; i++) {
        for (size_t j = 0; j < _dim; j++) 
          x[j] = *(_data + (*(m_neighbors + i)) * _dim + j);

        currCost += get_label(x, &label, cflag);
        counts[label]++;
        ...
        for (size_t i = 0; i < cluster_num; i++) {
          if (counts[i] > 0) {
            ...
            std::memcpy(m_means[i], next_means[i], sizeof(float) * _dim);
          }
        }
    ...
    if (!togg_in.is_open()) {
      ...
      for (size_t i = 0; i < _nd; i++) {
        ...
        if (size > cluster_num) {
          ...
          Cluster(_final_graph[i].data(), size, labels);
          ...
          for (size_t j = 0; j < tmp_c_num || it != mset.end(); j++) {
            ...
            for (size_t k = 0; k < size; k++) {
              if (labels[k] == *it) {
                float temp = togg_dist->compare((float*)(_data + _final_graph[i][k] + _dim), m_means[*it], _dim);
                if (temp < min_dist || min_dist == -1) {
                  min_dist = temp;
                  new_cluster_center[*it] = _final_graph[i][k];
                }
              }
            }
            nc_[i][j].push_back(new_cluster_center[*it]);
            if (j != *it) {
              for (size_t u = 0; u < size; u++) {
                if (labels[u] == *it)
                  labels[i] = j;
          ...
          for (size_t j = 0; j < size; j++) {
            if (nc_[i][labels[j]][0] != _final_graph[i][j])
              nc_[i][labels[j]].push_back(_final_graph[i][j]);
          }
          delete labels;
        }
        else {
          nc_[i].resize(1);
          for (size_t j = 0; j < size; j++)
            nc_[i][0].push_back(_final_graph[i][j]);
        }
      }
      ...
      while (!togg_in.eof()) {
        ...
        togg_in.read((char*)&cluster_num_, sizeof(unsigned));
        ...
        for (unsigned j = 0; j < cluster_num_; j++) {
          unsigned cnum;
          togg_in.read((char *)&cnum, sizeof(unsigned));
          tmp[j].resize(cnum);
          togg_in.read((char *)tmp[j].data(), cnum * sizeof(unsigned));
        }
        nc_.push_back(tmp);
        ++nodes;
      }
...
#pragma omp parallel for schedule(dynamic, 65536)
  ...
  void Index<T, TagT>::optimize_graph() {  // use after build or load
    ...
    _neighbor_len = (_width + cluster_num + 1) * sizeof(unsigned);
    ...
    for (unsigned i = 0; i < _nd; i++) {
      ...
      for (unsigned j = 0; j < cluster_num_; j++) {
        unsigned cnum = nc_[i][j].size();
        std::memcpy(cur_node_offset, nc_[i][j].data(), cnum * sizeof(unsigned));
        cur_node_offset += cnum * sizeof(unsigned);
...
//    boost::dynamic_bitset<> flags{_nd, 0};
    ...
    while (k < (int) L) {
      ...
      if (retset[k].flag) {
        ...
        unsigned *op_cluster = NULL;
        unsigned cluster_offset = 0;

        for (size_t i = 0; i < cluster_num_; i++) {
          unsigned id = *(neighbors + cluster_offset);
          float norm = *((const float*)(_opt_graph + id * _node_size));
          float temp = togg_dist->compare((const float*)query, (const float*)(_opt_graph + id * _node_size + sizeof(float)), norm, (unsigned)_dim);
          flags[id] = 1;
          dist_cout++;
          if (temp < retset[L - 1].distance) {
